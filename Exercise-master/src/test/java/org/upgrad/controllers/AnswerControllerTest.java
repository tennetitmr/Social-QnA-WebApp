...
//    }
    ...
    public void getAllAnswersToQuestionsWithAuthentication() throws Exception {
        ...
        given ( answerService.getAllAnswersToQuestion ( 3 ) ).willReturn ( answers );
        String url = "/api/answer/all/3";
        mvc.perform ( get ( url ).session ( session ).contentType ( MediaType.asMediaType ( APPLICATION_JSON ) ) ).andExpect ( status ().is2xxSuccessful () ).andExpect ( jsonPath ( "$", hasSize ( 1 ) ) ).andExpect ( jsonPath ( "$[0].ans", Matchers.is ( answer.getAns () ) ) );
    }
