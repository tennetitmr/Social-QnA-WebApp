...
import org.upgrad.controller.CommonController;
...
public class CommonControllerTest {
    ...
    public void allCategories() throws Exception {
        ...
        mvc.perform ( get ( url ).contentType ( MediaType.asMediaType ( APPLICATION_JSON ) ) ).andExpect ( status ().is2xxSuccessful () ).andExpect ( jsonPath ( "$", hasSize ( 1 ) ) ).andExpect ( jsonPath ( "$[0].title", Matchers.is ( category.getTitle () ) ) );
    }
    ...
    public void allQuestions() throws Exception {
        ...
        mvc.perform ( get ( url ).contentType ( MediaType.asMediaType ( APPLICATION_JSON ) ) ).andExpect ( status ().is2xxSuccessful () ).andExpect ( jsonPath ( "$", hasSize ( 1 ) ) ).andExpect ( jsonPath ( "$[0].content", Matchers.is ( question.getContent () ) ) );
    }
